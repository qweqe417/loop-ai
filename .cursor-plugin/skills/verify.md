...
```bash
python CURSOR_PLUGIN_ROOT_PLACEHOLDER/engines/run.sh verify --scenario <scenario-id> --format json
```
...
- Use Context Router to load the failed file:
  ```bash
  python CURSOR_PLUGIN_ROOT_PLACEHOLDER/engines/run.sh context route --stage repair --format json
  ```
- Find root cause
- Apply minimal fix
- Re-run verification
