...
> **引擎路径**: 使用 `{plugin_root}` 定位插件安装目录。如果该变量未设置，
> 读取 `{aicode_dir}/plugin-root.txt` 获取插件实际路径，替换下面的 `{plugin_root}`。
...
```bash
{engines_cmd} init --scan-only --format json
```

Or on Windows:
```bat
{engines_cmd_win} init --scan-only --format json
```
...
```bash
{engines_cmd} init --generate --auto-confirm --format json
```
